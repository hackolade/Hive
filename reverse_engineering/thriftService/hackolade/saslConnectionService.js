...
const kerberosAuthentication = (kerberosAuthProcess, connection, logger) => options =>
	new Promise((resolve, reject) => {
		const inst = new kerberosAuthProcess(options.krb_host, options.port, options.krb_service);
		let transition = 0;
		...
		inst.init(options.username, options.password, (err, client) => {
			...
			const onData = data => {
				...
				if (result === OK) {
					...
					logger.log(
						'kerberos authentication. Step 3. Transition #' +
							transition +
							': completed. Chosen QOP: ' +
							getQopName(qualityOfProtection),
					);

					onSuccess();
				} else {
					logger.log(
						'kerberos authentication. Step 3. Transition #' + transition + ': failed. Code: ' + result,
					);
...
const ldapAuthentication = connection => options =>
	new Promise((resolve, reject) => {
		connection.connect();
