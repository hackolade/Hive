...
const promisifyCallback = operation =>
...
const connect =
	...
	(TCLIService, TCLIServiceTypes, logger, kerberos) => {
		...
		const getTables = (sessionHandle, schemaName, tableTypes) =>
			new Promise((resolve, reject) => {
				const request = new TCLIServiceTypes.TGetTablesReq({
					sessionHandle,
					schemaName,
					tableTypes,
					tableName: '_%',
				...
				getConnection().then(client =>
					client.GetTables(request, (err, res) => {
						if (err) {
							reject(err);
						} else {
							resolve(res);
		...
		const getTableTypes = sessionHandle =>
			new Promise((resolve, reject) => {
				const request = new TCLIServiceTypes.TGetTableTypesReq({ sessionHandle });

				getConnection().then(client =>
					client.GetTableTypes(request, (err, res) => {
						if (err) {
							reject(err);
						} else {
							resolve(res);
		...
		const getTypeInfo = sessionHandle =>
			new Promise((resolve, reject) => {
				const request = new TCLIServiceTypes.TGetTypeInfoReq({ sessionHandle });

				getConnection().then(client =>
					client.GetTypeInfo(request, (err, res) => {
						if (err) {
							reject(err);
						} else {
							resolve(res);
		...
		const getColumns = (sessionHandle, schemaName, tableName) =>
			new Promise((resolve, reject) => {
				const request = new TCLIServiceTypes.TGetColumnsReq({ sessionHandle, schemaName, tableName });

				getConnection().then(client =>
					client.GetColumns(request, (err, res) => {
						if (err) {
							reject(err);
						} else {
							resolve(res);
		...
		const fetchResultRequest = options =>
			new Promise((resolve, reject) => {
		...
		const getOperationStatus = operationHandle =>
			new Promise((resolve, reject) => {
		...
		const waitFinish = operationHandle =>
			new Promise((resolve, reject) => {
				const repeat = count => {
			...
			client =>
				new Promise((resolve, reject) => {
					...
					client.OpenSession(request, (err, session) => {
						const closeSession = () => {
							return new Promise((resolve, reject) => {
								const request = new TCLIServiceTypes.TCloseSessionReq({
						...
						if (err) {
							...
							logger.log(
								'Status: ' +
									...
									'\nInfo Message: ' +
									(session.status.infoMessages || []).join('\n'),
							);
